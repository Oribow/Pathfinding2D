...
namespace NavGraph.Build
{
    ...
    /// </summary>
    public class GeometrySet : IVisualizable
    {
        ...
        public Vector2[][] ToVertexArray()
        {
            Vector2[][] result = new Vector2[edgeList.Count + polygonList.Count][];
            for (int iPoly = 0; iPoly < polygonList.Count; iPoly++)
            {
                result[iPoly] = polygonList[iPoly];
            }
            for (int iEdge = 0; iEdge < edgeList.Count; iEdge++)
            {
                result[iEdge + polygonList.Count] = edgeList[iEdge];
            }
            return result;
        }
